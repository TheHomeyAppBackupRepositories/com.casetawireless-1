...
class LutronClient extends events_1.EventEmitter {
    constructor(homey, address, secrets) {
        ...
        this.registeredMotionSensors = false;
        ...
        this.leapClient.on('disconnected', () => {
            ...
            this.reconnectInterval = this.homey.setInterval(async () => {
                try {
                    ...
                    if (this.registeredMotionSensors) {
                        await this.subscribeToOccupancy(true);
                    }
                }
                catch (e) {
    ...
    async subscribeToButton(button) {
        ...
        await this.leapClient.subscribe(button.href + '/status/event', r => {
            if (r.Body) {
                this.debug('Button status event received', JSON.stringify(r.Body));
    ...
    async subscribeToOccupancy(reconnect) {
        if (this.registeredMotionSensors && !reconnect) {
            // Already subscribed, skip
            return;
        }
        this.registeredMotionSensors = true;
        await this.leapClient.subscribe('/occupancygroup/status', r => {
            if (r.Body) {
                this.debug('Occupancy group event received', JSON.stringify(r.Body));
                const occupancyGroupStatuses = r.Body.OccupancyGroupStatuses;
                occupancyGroupStatuses.forEach(s => this.emit('occupancyEvent', s));
                return;
            }
            this.log('Unknown occupancy event', JSON.stringify(r));
        });
    }
